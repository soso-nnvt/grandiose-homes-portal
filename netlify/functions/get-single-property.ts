...
export const handler: Handler = async (event) => {
  ...
  if (!id) {
    return {
      statusCode: 200,
      body: JSON.stringify({ error: true, message: 'Property ID is required' }),
    };
  }
  ...
  try {
    const response = await fetch(`${WP_BASE_URL}property/${id}?_embed`, {
      headers: {
        'Authorization': auth,
      },
    });
